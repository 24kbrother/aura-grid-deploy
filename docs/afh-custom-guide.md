...
在画布上，当解开大屏物理锁定后，您可以在可视化界面中通过拖动和坐标网格调整悬浮微件的定位。布局编辑界面如下图所示：

![AFH布局编辑界面](assets/afh-editor.png)
> **图 3.1**：Aura Floating Hub 可视化编辑网格与绝对定位拖拽界面。
...
下面是测试通过的自定义浮动微件在 3D 户型图画布之上的实际融合与渲染效果图：

![浮动微件渲染效果](assets/afh-render.png)
> **图 5.1**：气象微悬浮球与全屋灯控 Deck 模版在 3D 大屏画布上的动态运行效果。
